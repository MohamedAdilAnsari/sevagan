  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/send-otp', {
        ...
        body: JSON.stringify({ mobile: phone, purpose: 'donor_registration' })
      });
      const data = await res.json();
      if (data && data.otp) generatedOtp = data.otp;
    } catch (err) {
      console.warn('Failed to send OTP via API, using default code:', err);
    }
    ...
    openModal('otp', {
      ...
      onVerifySuccess: async () => {
        ...
        openModal('success', {
          title: donorFor === 'me' ? 'Profile Published! 🎉' : 'Donor Registered! 🎉',
          message: donorFor === 'me' 
            ? 'Congratulations! Mobile number verified. You are now a registered SEVAGAN blood donor.' 
            : 'Your friend has been successfully verified & registered on SEVAGAN Blood Network.',
          nextAction: () => navigateToSection(donorFor === 'me' ? 'profile' : 'find-donors')
        });
      }
    });
  };
