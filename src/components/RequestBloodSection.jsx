  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/send-otp', {
        ...
        body: JSON.stringify({ mobile, purpose: 'blood_request' })
      });
      const data = await res.json();
      if (data && data.otp) generatedOtp = data.otp;
    } catch (err) {
      console.warn('Failed to send OTP via API:', err);
    }
    ...
    openModal('otp', {
      ...
      onVerifySuccess: async () => {
        ...
        openModal('success', {
          ...
          message: 'Mobile number verified! Emergency request has been broadcasted to all nearby voluntary donors.',
